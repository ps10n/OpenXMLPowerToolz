            ...
            using (OpenXmlMemoryStreamDocument streamDoc = new OpenXmlMemoryStreamDocument(wmlDoc))
            {
                using (WordprocessingDocument wDoc = streamDoc.GetWordprocessingDocument())
                {
                    ...
                    if (!string.IsNullOrEmpty(foreColor))
                    {
                        ...
                        if (colorValue == 0)
                            throw new OpenXmlPowerToolzException(String.Format("Add-DocxText: The specified color {0} is unsupported, Please specify the valid color. Ex, Red, Green", foreColor));
...
AAsACwDBAgAAbCwAAAAA";
                                ...
                                using (WordprocessingDocument defaultDotx = WordprocessingDocument.Open(memoryStream, true))
                                {
                                    ...
                                    if (templateStyle == null)
                                        throw new OpenXmlPowerToolzException(String.Format("Add-DocxText: The specified style name {0} is unsupported, Please specify the valid style. Ex, Heading1, Heading2, Title", styleName));
                                    else
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                using (WordprocessingDocument wDoc = WordprocessingDocument.Open(memoryStream, true))
                {
                    ...
                    if (outputDirectory != null && outputDirectory != string.Empty)
                    {
                        ...
                        if (!di.Exists)
                        {
                            throw new OpenXmlPowerToolzException("Output directory does not exist");
                        }
