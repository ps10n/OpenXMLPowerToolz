...
namespace OpenXmlPowerToolz.Previous
{
        ...
        private static void ChangeFootnoteEndnoteReferencesToUniqueRange(WordprocessingDocument wDoc, int startingIdForFootnotesEndnotes)
        {
            ...
            foreach (var r in references)
            {
                ...
                if (r.Name == W.footnoteReference)
                {
                    ...
                    if (fn == null)
                        throw new OpenXmlPowerToolzException("Invalid document");
...
@"<w:style w:type=""character""
            ...
            if (element != null)
            {
                if (element.Name == W.r)
                {
                    ...
                    if (statusList.Count() > 1)
                        throw new OpenXmlPowerToolzException("Internal error - have both deleted and inserted text elements in the same run.");
        ...
        private static void TestForInvalidContent(WordprocessingDocument wDoc)
        {
            foreach (var part in wDoc.ContentParts())
            {
                ...
                if (xDoc.Descendants(W.altChunk).Any())
                    throw new OpenXmlPowerToolzException("Unsupported document, contains w:altChunk");
                if (xDoc.Descendants(W.subDoc).Any())
                    throw new OpenXmlPowerToolzException("Unsupported document, contains w:subDoc");
                if (xDoc.Descendants(W.contentPart).Any())
                    throw new OpenXmlPowerToolzException("Unsupported document, contains w:contentPart");
            }
        }
            ...
            var grouped = list
                .GroupBy(sr =>
                {
                    ...
                    if (level >= sr.AncestorElements.Length)
                        throw new OpenXmlPowerToolzException("Internal error 4 - why do we have ComparisonUnitAtom objects with fewer ancestors than its siblings?");
            ...
            var elementList = grouped
                .Select(g =>
                {
                    ...
                    if (level >= g.First().AncestorElements.Length)
                        throw new OpenXmlPowerToolzException("Internal error 3 - why do we have ComparisonUnitAtom objects with fewer ancestors than its siblings?");
    ...
    internal class ComparisonUnitGroup : ComparisonUnit
    {
        ...
        public ComparisonUnitGroup(IEnumerable<ComparisonUnit> comparisonUnitList, ComparisonUnitGroupType groupType, int level)
        {
            ...
            if (ancestor == null)
                throw new OpenXmlPowerToolzException("Internal error: ComparisonUnitGroup");
        ...
        public static ComparisonUnitAtom GetFirstComparisonUnitAtomOfGroup(ComparisonUnit group)
        {
            ...
            while (true)
            {
                ...
                if (tw == null)
                    throw new OpenXmlPowerToolzException("Internal error: GetFirstComparisonUnitAtomOfGroup");
