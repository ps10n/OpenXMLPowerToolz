            ...
            if (settings.ContentTypeRegexExtension != null)
            {
                foreach (var ext in settings.ContentTypeRegexExtension.Root.Elements("Extension"))
                {
                    ...
                    if (ruleToUpdate == null)
                        throw new OpenXmlPowerToolzException("ContentTypeRexexExtension refers to content type that does not exist");
...
<w:styles xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" xmlns:w15="http://schemas.microsoft.com/office/word/2012/wordml" xmlns:w16se="http://schemas.microsoft.com/office/word/2015/wordml/symex" mc:Ignorable="w14 w15 w16se">
	<w:docDefaults>
		...
		<w:pPrDefault/>
            ...
            if (body == null)
                throw new OpenXmlPowerToolzException("Internal error: invalid document");
        ...
        private static XElement HierarchyPerSettings(XElement contentTypeXml, WmlToXmlSettings settings)
        {
            ...
            if (rootElement == null)
                throw new OpenXmlPowerToolzException("Invalid content type hierarchy definition - no root element");
            ...
            foreach (var item in contentTypeXml.Elements())
            {
                if (!hierarchyElements.Contains(item.Name))
                    throw new OpenXmlPowerToolzException(string.Format("Invalid Content Type Hierarchy Definition - missing def for {0}", item.Name));
                ...
                if (hierarchyElements.Contains(item.Name))
                {
                    while (true)
                    {
                        ...
                        if (stack.Count() == 0)
                            throw new OpenXmlPowerToolzException("Internal error = reached top of hierarchy - prob not an internal error - some other error");
                    }
                    continue;
                }
        ...
        public static object ProduceXmlTransform(OpenXmlPart part, XNode node, WmlToXmlSettings settings)
        {
            ...
            if (element != null)
            {
                ...
                if (contentType == null && element.Name == W.r)
                {
                    ...
                    {
                        throw new OpenXmlPowerToolzException("Entry for Run content type in XML generation lambdas is required");
                    }
                }
                ...
                if (element.Name == W.hyperlink)
                {
                    ...
                    {
                        throw new OpenXmlPowerToolzException("Entry for Hyperlink content type in XML generation lambdas is required");
                    }
                }
            ...
            foreach (var rlc in runContent)
            {
                if (rlc.Name == W.r || rlc.Name == W.sdt)
                {
                    ...
                    foreach (var rule in runContentTypeRuleList)
                    {
                        ...
                        if (rule.RegexArray != null)
                            throw new OpenXmlPowerToolzException("Invalid Run ContentType Rule - Regex not allowed");
                        if (rule.MatchLambda != null)
                        {
